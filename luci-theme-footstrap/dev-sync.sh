...
# nothing (a copied tree without .git). An `&&` used to guard an ASSIGNMENT is a different shape and
# is safe — stage.sh:41 and update-po.sh:39 rely on that. expr refuses a tag with sed/shell-special
# characters rather than interpolating it.
...
# either: owfeed bundles the catalogues inside the theme package (owfeed.yml), so a dev router gets
# them from nowhere unless this sync makes them. po2lmo is a luci-base host tool, absent from a dev
# machine by default, so this is best-effort: without it the strings stay English and the sync still
...
#
# THE BASENAME IS `footstrap-theme`, matching owfeed.yml and NOT the po file's own name: lmo_load_
# catalog globs *.<lang>.lmo so any basename loads, and a v0.8.4 router still owns footstrap.ru.lmo
# through the old luci-i18n package, where the same path would be a file conflict. Sync under a
# different name and the dev router carries two archives for the same msgids, with glob order
# deciding which one the page shows.
